...
#############################################################
# Classification Model - Linear Discriminant Analysis (LDA) #
#############################################################
...
# ensemble model (1j).
LDA_prediction <- data.frame(
  "LDA" = real_vs_pred$Prediction
)

write.csv(LDA_prediction,
          "output\\LDA_prediction.csv",
          row.names = TRUE)

# We'll also store the probabilities associated with these predictions for
# evaluating said ensemble model (ij).
lda_probs <- data.frame(
  "LDA" = probs
)
write.csv(lda_probs,
          "output\\lda_probs.csv",
          row.names = TRUE)
...
roc_curve <- roc(response = test_labels$gender,
                 ...
                 auc = TRUE,
                 percent = TRUE,
                 main = "ROC for Linear Discriminant Analysis Model",
                 xlab = "False Positive Percentage (1 - Specificity)",
                 ylab = "True Positive Percentage (Sensitivity)",
                 legacy.axes = TRUE)
roc_curve # An AUC of 95.27% means we can predict the classification 95.27% of
...
# of the models by viewing files 1a - 1f, 1h - 1i or see how all the models work

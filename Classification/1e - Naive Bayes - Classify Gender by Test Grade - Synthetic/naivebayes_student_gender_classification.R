...
library(e1071) # for Naive Bayes probabalistic model
...
probs <- predict(mod, data_test, type = 'raw')[,2] # [,2] only interested in
...
preds <- predict(mod, data_test, type = 'class')
...
# ensemble model (1j).
nBayes_prediction <- data.frame(
  "nBayes" = real_vs_pred$Prediction
)

write.csv(nBayes_prediction,
          "output\\nBayes_prediction.csv",
          row.names = TRUE)

# We'll also store the probabilities associated with these predictions for
# evaluating said ensemble model (ij).
nBayes_probs <- data.frame(
  "nBayes" = probs
)
write.csv(nBayes_probs,
          "output\\nBayes_probs.csv",
          row.names = TRUE)
...
roc_curve <- roc(response = test_labels$gender,
                 ...
                 auc = TRUE,
                 percent = TRUE,
                 main = "ROC for Naive Bayes Model",
                 xlab = "False Positive Percentage (1 - Specificity)",
                 ylab = "True Positive Percentage (Sensitivity)",
                 legacy.axes = TRUE)
roc_curve # An AUC of 80.45% means we can predict the classification 80.45% of
...
# of the models by viewing files 1a - 1d, 1f - 1i or see how all the models work

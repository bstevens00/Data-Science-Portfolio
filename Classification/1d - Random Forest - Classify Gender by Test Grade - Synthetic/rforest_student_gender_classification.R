...
# StudentsPerformance.csv data. It is a possible next step after that initial
# analysis. You may begin here, but all visualizations of the data will be
# absent. For a fuller view of this data, view the file below:
...
# This time up is a Random Forest model. A Random Forest model is essentially
# taking multiple simple trees and then merging them together to create a
# "Forest". It is "Random", because every simple tree model is created from
# taking a random sample of the data, with replacement (called bootstrapping),
# and then training a model. Then, going a step further than bootstrapping,
# which is already a strong method of decreasing variance in a model, a random
# subset of predictors is chosen at each stage to fit a classification.

# Said another way, we train a model from a randomly sized, randomly chosen
# observation pool, some of them possibly repeated (that's what "with
# replacement" means), and then we create a model that uses a randomly selected
# number of the predictors, the predictors themselves of which are also chosen
# at random. Then we do this again for many models in a row. Then we merge them.

# In almost all cases, Random Forest models are superior to their simple tree
# equivalents. The only case where it generally fails to be superior is in the
# visual interpretation, as there is no way to graph this. It is strictly for
# predictive purposes. This means that you can't really "explain" the model to
# someone, they'd have to understand how a Random Forest model opperates.
...
mod <- randomForest(gender ~ mathScore + readingScore + writingScore,
                    data = data_train) # here is the model
probs <- predict(mod, data_test, type = 'prob')[,2] # [,2] only interested in
# probability the student is classified as male
...
# ensemble model (1j).
rforest_prediction <- data.frame(
  "rforest" = predictions
)

write.csv(rforest_prediction,
          "output\\rforest_prediction.csv",
          row.names = TRUE)

# We'll also store the probabilities associated with these predictions for
# evaluating said ensemble model (ij).
rforest_probs <- data.frame(
  "rforest" = probs
)
write.csv(rforest_probs,
          "output\\rforest_probs.csv",
          row.names = TRUE)
...
roc_curve <- roc(response = test_labels$gender,
                 ...
                 auc = TRUE,
                 percent = TRUE,
                 main = "ROC for Random Forest Model",
                 xlab = "False Positive Percentage (1 - Specificity)",
                 ylab = "True Positive Percentage (Sensitivity)",
                 legacy.axes = TRUE)
roc_curve # An AUC of 91.92% means we can predict the classification 91.92% of
...
# of the models by viewing files 1a - 1c, 1e - 1i or see how all the models work

...
# Import the .csv predictions from all the single models
library(readr)
...
roc_curve <- roc(response = knn_prediction$Observed, # the actual scores
                 ...
                 legacy.axes = TRUE)
roc_curve # An AUC of 95.32% means we can predict the classification 95.32% of
...
# First up, KNN
roc(response = knn_prediction$Observed, # the actual scores
    predictor = ensemble_probabilities, # the associated probs
    plot = TRUE,
    auc = TRUE,
    percent = TRUE,
    ...
    ylab = "True Positive Percentage (Sensitivity)",
    legacy.axes = TRUE)
plot.roc(x = knn_prediction$Observed,
         predictor = knn_probs$KNN, # KNN probs
...
plot.roc(x = knn_prediction$Observed,
         ...
         col = "aquamarine4")
plot.roc(x = knn_prediction$Observed,
         predictor = logistic_probs$logistic, 
         add = TRUE, 
         percent = 100,
         print.auc = TRUE,
         print.auc.x = 22,
         ...
         col = "blueviolet") 
plot.roc(x = knn_prediction$Observed,
         predictor = rforest_probs$rforest, 
         add = TRUE, 
         percent = 100,
         print.auc = TRUE,
         print.auc.x = 22,
